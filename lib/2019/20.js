...
const bfs2 = (board, portalPositions, start, end) => {
  const isInner = ({ x, y }) =>
    x !== 2 && y !== 2 && x !== board[0].length - 3 && y !== board.length - 3

  let candidates = [{ ...start, steps: 0, depth: 0 }],
    visited = {}

  while (candidates.length > 0) {
    const candidate = candidates.splice(0, 1)[0]
    const { x, y, steps, depth } = candidate
    const candidateKey = `${getCellKey(candidate)}-${depth}`
    ...
    candidates.push(
      ...[
        { x: x - 1, y, steps: steps + 1, depth },
        { x: x + 1, y, steps: steps + 1, depth },
        { x, y: y - 1, steps: steps + 1, depth },
        { x, y: y + 1, steps: steps + 1, depth },
      ]
        .filter(({ x, y }) => {
          const tile = board[y][x]
          ...
          if (isPortal(tile)) {
            const destinations = portalPositions[tile].length

            if (destinations === 1 && depth === 0) {
              return true
            }

            if (destinations === 2 && depth > 0) {
              return true
            }

            if (destinations === 2 && depth === 0 && isInner({ x, y })) {
              return true
            }
        ...
        .map((pos) => {
          ...
          if (isPortal(tile) && portalPositions[tile].length === 2) {
            ...
            return {
              ...newPosition,
              steps: pos.steps + 1,
              depth: pos.depth + (isInner({ x: pos.x, y: pos.y }) ? 1 : -1),
            }
          }
...
const part2 = (input) => {
  ...
  return bfs2(grid, portalPositions, portalPositions.AA[0], 'ZZ')
}
