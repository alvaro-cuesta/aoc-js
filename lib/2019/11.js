...
const paintHull = R.curry((initialColor, input) => {
  const hull = { 0: { 0: initialColor } }
  let pos = [0, 0]
  let dir = [0, -1]

  const brain = intcode.spawn(input)

  for (;;) {
    {
      const { value, done } = brain.next()
      ...
      assert.deepStrictEqual(
        value,
        { type: 'input' },
        'Expected request for input',
      )
    }

    {
      const { value, done } = brain.next(getHullColor(hull, pos))
      assert.deepStrictEqual(done, false, 'Expected request for output (1)')
      assert.deepStrictEqual(
        value.type,
        'output',
        'Expected request for output (1)',
      )

      setHullColor(hull, pos, value.value)
    }

    {
      const { value, done } = brain.next()
      assert.deepStrictEqual(done, false, 'Expected request for output (2)')
      assert.deepStrictEqual(
        value.type,
        'output',
        'Expected request for output (2)',
      )

      dir = value.value === 1 ? cw(dir) : ccw(dir)
      pos = v2add(pos, dir)
    }
  }
})
