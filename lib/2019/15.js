...
const makeGetTile =
  ({ w, h, oxygen: [ox, oy], threshold, data }) =>
  ([x, y]) => {
    const evenX = x % 2 === 0
    const evenY = y % 2 === 0
...
const makeDijsktra = (data) => {
  ...
  return (pos) => {
    const remaining = [{ pos, steps: 0 }]
    const visited = {}

    while (remaining.length > 0) {
      const { pos, steps } = remaining.shift()
      ...
      visited[posKey] = steps

      const neighbors = DIRECTIONS.map((dir) => ({
        pos: vec.add(pos, dir),
        steps: steps + 1,
      })).filter(
        ({ pos: newPos, steps: newSteps }) =>
          getTile(newPos) !== TILES.WALL &&
          (visited[newPos] === undefined || visited[newPos] > newSteps),
      )
