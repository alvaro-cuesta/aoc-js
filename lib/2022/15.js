...
const solvePart2 = R.curry((max, input) => {
  ...
  for (let y = 0; y <= max; y++) {
    for (let x = 0; x <= max; ) {
      const inRangeOfSensors = sensors.filter(
        (s) => manhattanDist([s.sX, s.sY], [x, y]) <= s.d,
      )

      const isBeacon = sensors.some((s) => s.bX === x && s.bY === y)

      if (inRangeOfSensors.length === 0 && !isBeacon) {
        return x * 4000000 + y
      }

      const xd = inRangeOfSensors
        .map((s) => s.d + s.sX - x - Math.abs(s.sY - y) + 1)
        .reduce(R.max, -Infinity)
