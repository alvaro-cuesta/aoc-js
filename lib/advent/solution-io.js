const { downloadInput } = require("./input");
...
const partIO = (year, day, part, session) =>
  makeSolutionIO(downloadInput(year, day, session))(year, day, part);

const dayIO = (year, day, session) => {
  const solutionIO = makeSolutionIO(downloadInput(year, day, session));
